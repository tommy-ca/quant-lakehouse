...
Pure extract+load: no file listing, no caching, no scheduling.
Prefect resolves file lists (via ``ArchiveFileCache`` or S3 listing)
and passes them to this resource.

Per-file flow:
1. Download ZIP content via aiohttp
2. Parse CSV inside each ZIP (accounting for data-type-specific format)
3. Yield rows with merge-on-primary-key for idempotent loading
"""
